...
export async function fetchEntriesPage(page = 0) {
    ...
    try {
        ...
        if (isPublicVisitor) {
            query = query.limit(PUBLIC_FEED_LIMIT);
        } else {
        ...
        if (isPublicVisitor && formatted.length > PUBLIC_FEED_LIMIT) {
            formatted = formatted.slice(0, PUBLIC_FEED_LIMIT);
        }
...
export async function loadNextPage() {
    if (!state.authenticatedUser || !state.hasMorePages || state.isFetchingPage) return;
